...
"""
import datetime, os, requests, shutil, sys, struct, transmission, xattr
...
def clean_torrents(torrents_dir, days_seeding=DAYS_SEEDING):
	...
	# Get torrents.
	try:
		torrents = client('torrent-get',
			fields=['downloadDir', 'id', 'name', 'secondsSeeding'])['torrents']
	...
	for torrent in torrents:
		...
		if torrent['downloadDir'].startswith(torrents_dir):
			...
			if torrent['secondsSeeding'] >= seconds_seeding:
				sys.stdout.write('Removing old torrent: %s\n' % torrent['name'])
				client('torrent-remove',
...
def main():
	if len(sys.argv) not in [2, 3]:
		err('Usage: %s <torrents_dir> [days_seeding=%s]' % (
